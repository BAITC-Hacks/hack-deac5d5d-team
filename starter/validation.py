...
def text_values(series, label, limit=None, with_number=False):
    require(
        series.map(lambda s: isinstance(s, str) and bool(s.strip())).all(),
...
def validate_inputs(edges, nodes, tx, profile=None, *, return_dataset=False):
    from contracts import Dataset, InputProfile
    from input_contracts import validate_depths, validate_profile

    profile = profile or InputProfile()
    ...
    require(
        nodes.depth.le(profile.max_depth).all(), f"nodes.depth: expected 0..{profile.max_depth}"
    )
    ...
    require(
        edges.depth.le(profile.max_depth).all(), f"edges.depth: expected 1..{profile.max_depth}"
    )
    ...
    validate_depths(edges, nodes, profile.max_depth)
    validate_profile(tx, profile)
    if return_dataset:
        prepared_edges, prepared_tx = edges.copy(), tx.copy()
        prepared_edges["sum_tiyn"] = pd.Series(
            expected.edge_cents.tolist(), index=edges.index, dtype=object
        )
        prepared_tx["sum_tiyn"] = pd.Series(amounts.cents.tolist(), index=tx.index, dtype=object)
        prepared_tx["date"] = dates
        prepared_tx["day"] = dates.dt.date
        return Dataset(prepared_edges, nodes.copy(), prepared_tx)
...
def validate_outputs(roles, clusters, top, nodes, edges):
    ...
    for frame, required, label in (
        (roles, ROLE_COLUMNS, "nodes_roles"),
    ...
    require(
        roles.gid.is_unique and set(roles.gid) == set(nodes.gid),
    ...
    for frame, label, cols in (
        (roles, "nodes_roles", ("role_score", "priority_score")),
    ...
    require(
        set(roles.cluster_id) == set(clusters.cluster_id), "clusters: cluster references mismatch"
    )
    ...
    for edge, amount in zip(edges.itertuples(index=False), cents):
        require(
            edge.src in assignment.index and edge.dst in assignment.index,
    ...
    for cluster, amount in zip(clusters.itertuples(index=False), internal):
        ...
        require(
            isinstance(gids, list) and len(gids) > 0, "clusters.top_gids: expected nonempty list"
        )
        require(
            all(isinstance(g, str) and g in {str(v) for v in group} for g in gids),
    ...
    require(
        top.gid.is_unique and set(top.gid) <= set(roles.gid), "top_nodes: duplicate/unknown IDs"
    )
    require(
        top["rank"].tolist() == list(range(1, len(top) + 1)),
        "top_nodes: ranks must be 1..N in order",
    )
    ...
    require(
        top.priority_score.tolist() == lookup.priority_score.tolist(),
        "top_nodes: scores disagree with nodes_roles",
    )
    require(
        top.priority_score.tolist() == roles.priority_score.nlargest(len(top)).tolist(),
...
def validate_output_files(out_dir: Path, nodes, edges):
    """Validate one directory-bound CSV/JSON/manifest generation, including legacy."""
    from storage import validate_bundle

    validate_bundle(out_dir, nodes, edges)
