...
Leiden is an experiment dependency, not a production pipeline dependency:
  ...
  python experiments/compare_communities.py --data ../data --out COMMUNITY_EXPERIMENT.md
"""
...
from communities import assign_clusters, louvain_strategy, undirected_projection
...
def build_report(
    ...
    from features import build_graph, external_graph
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("COMMUNITY_EXPERIMENT.md"))
    parser.add_argument(
        "--with-roles",
        action="store_true",
