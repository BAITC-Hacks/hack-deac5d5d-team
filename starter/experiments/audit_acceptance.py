...
Reads the specified Parquet dataset and source files. Only --output, when
explicitly supplied, is written outside a TemporaryDirectory. Exit status is
...
from communities import undirected_projection
from contracts import AnalysisResult
from pipeline import load_dataset, prepare_dataset, run_analysis
from storage import save_result, validate_bundle, write_graph_json
...
def legacy_reader_probe(root: Path, result: AnalysisResult) -> dict:
    ...
    with patch("storage.pd.read_csv", side_effect=interleave):
