...
from hackalem.validation import (
    ValidationError,
    validate_inputs,
    validate_output_files,
    validate_outputs,
)
from starter.starter import basic_features, build_graph
...
def fixture():
    ...
    depths[18], depths[19] = 1, 2
    depths[14] = depths[15] = depths[16] = 3
    nodes = pd.DataFrame(
    ...
    tx = pd.DataFrame(
        {
            "src": [gids[src] for src, _, _, _ in transfers],
...
class AnalysisTests(unittest.TestCase):
    def test_motifs_and_incomplete_observations(self):
        ...
        self.assertLessEqual(clipped.role_score, 0.2)
    ...
    def test_incoming_outgoing_sum_and_exact_internal_turnover(self):
        ...
        reverse["sum_kzt"] = 0.02
        t = pd.concat([t, reverse], ignore_index=True)
        e = t.groupby(["src", "dst"], as_index=False).agg(
            sum_kzt=("sum_kzt", "sum"), n_tx=("sum_kzt", "size")
        )
        e["depth"] = 1
    ...
    def test_dates_require_previous_day_within_seven_days(self):
        e, n, tx = fixture()
        for day, expected in [(1, 0.0), (2, 1.0), (8, 1.0), (9, 0.0)]:
            t = tx.copy()
            ...
            with self.subTest(day=day):
                ...
                self.assertEqual(role == "transit", expected == 1.0)
        tx.loc[tx.dst.eq(BASE + 11), "date"] = pd.Timestamp("2026-07-10")
        self.assertEqual(temporal_support(tx)[BASE + 11], 0.0)
        ...
        tx = pd.DataFrame(
            {
                ...
                "sum_kzt": [100.0, 75.0, 25.0, 1000.0],
            }
        )
        self.assertEqual(temporal_support(tx)[2], 0.25)

    def test_shuffled_inputs_are_identical_and_ids_are_exact(self):
        e, n, t = fixture()
        first = run_analysis(e, n, t)
        second = run_analysis(
            e.sample(frac=1, random_state=5),
            n.sample(frac=1, random_state=6),
        ...
        for start in [1, 6, 11]:
            for src in range(start, start + 5):
                for dst in range(start, start + 5):
                    if src != dst:
                        graph.add_edge(src, dst, sum_kzt=100.0, n_tx=1, depth=1)
            for peer in [start, start + 1]:
                graph.add_edge(0, peer, sum_kzt=1.0, n_tx=1, depth=1)
                graph.add_edge(peer, 0, sum_kzt=1.0, n_tx=1, depth=1)
            ...
            [
                {
                    ...
                    "sum_kzt": attrs["sum_kzt"],
                }
                for src, dst, attrs in graph.edges(data=True)
            ]
        )
        features = analysis_features(
            graph, basic_features(graph, nodes), tx, assign_clusters(graph)
        )
    ...
    def test_module_and_legacy_cli_agree_and_validation_is_read_only(self):
        project = Path(__file__).resolve().parents[1]
        launchers = {
            "module": [sys.executable, "-B", "-m", "hackalem"],
            "legacy": [sys.executable, "-B", str(project / "starter" / "starter.py")],
        }
        expected = None
        for name, launcher in launchers.items():
            with self.subTest(launcher=name), tempfile.TemporaryDirectory() as temp:
                ...
                data = root / "data"
                data.mkdir()
                for filename, frame in [("edges", e), ("nodes", n), ("transactions", t)]:
                    frame.to_parquet(data / f"{filename}.parquet", index=False)
                # Module launch is canonical from the repository root. The legacy
                # absolute launcher must also work from an unrelated current folder.
                working_directory = project if name == "module" else root
                command = launcher + ["--data", str(data)]
                out = root / "output"
                run = subprocess.run(
                    command + ["--out", str(out)],
                ...
                # Runtime-independent deliverables must be identical through both entry points.
                actual = {key: value for key, value in snapshot.items() if key != "manifest.json"}
                if expected is None:
                    expected = actual
                else:
                    self.assertEqual(actual, expected)
                check = subprocess.run(
                    command + ["--validate-output", str(out)],
                    cwd=working_directory,
                    capture_output=True,
                    text=True,
                )
                ...
                e.loc[0, "sum_kzt"] = round(e.loc[0, "sum_kzt"] + 0.01, 2)
                e.to_parquet(data / "edges.parquet", index=False)
                bad = subprocess.run(
                    command + ["--out", str(out)],
                    cwd=working_directory,
                    capture_output=True,
