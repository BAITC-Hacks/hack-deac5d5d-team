...
from hackalem.analysis import amount_temporal_support, structural_betweenness
from hackalem.contracts import AnalysisConfig
from hackalem.validation import validate_output_files
from starter.starter import build_graph, write_outputs
...
class TemporalAmountTests(unittest.TestCase):
    def test_small_receipt_cannot_support_large_same_day_flow(self):
        tx = transfers(
            [
                (1, 2, "2026-07-15", 6600.0),
                (1, 2, "2026-07-22", 652000.0),
                (2, 3, "2026-07-22", 652000.0),
            ]
        )
        result = amount_temporal_support(tx)[2]
        self.assertEqual(result["temporal_matched_kzt"], 6600.0)
        self.assertEqual(result["same_day_uncertain_kzt"], 645400.0)
    ...
    def test_receipts_are_consumed_once_and_partial_coverage_counts(self):
        tx = transfers(
            [(1, 2, "2026-07-01", 100.0), (2, 3, "2026-07-02", 75.0), (2, 4, "2026-07-03", 75.0)]
        )
        result = amount_temporal_support(tx)[2]
        self.assertEqual(result["temporal_matched_kzt"], 100.0)
    ...
    def test_same_day_uncertainty_consumes_receipts_before_next_day(self):
        tx = transfers(
            [(1, 2, "2026-07-01", 100.0), (2, 3, "2026-07-01", 100.0), (2, 4, "2026-07-02", 100.0)]
        )
        result = amount_temporal_support(tx)[2]
        self.assertEqual(result["temporal_matched_kzt"], 0.0)
        self.assertEqual(result["same_day_uncertain_kzt"], 100.0)
        self.assertEqual(result["temporal_unmatched_out_share"], 0.5)
    ...
    def test_window_expiry_future_and_same_day_are_not_support(self):
        for outgoing, expected in [
            ...
            with self.subTest(day=outgoing):
                tx = transfers([(1, 2, "2026-07-01", 10.0), (2, 3, outgoing, 10.0)])
    ...
    def test_calendar_days_remain_correct_across_daylight_saving(self):
        tx = transfers([(1, 2, "2026-03-01", 10.0), (2, 3, "2026-03-09", 10.0)])
        tx["date"] = tx.date.dt.tz_localize("America/New_York")
        self.assertEqual(amount_temporal_support(tx)[2]["temporal_matched_kzt"], 0.0)
    ...
    def test_fifo_spends_old_receipt_before_newer_receipt(self):
        ...
        self.assertEqual(amount_temporal_support(tx)[2]["temporal_matched_kzt"], 30.0)
    ...
    def test_tiyn_precision_self_transfers_and_order_independence(self):
        tx = transfers(
            [
                (1, 2, "2026-07-01", 0.01),
                (1, 2, "2026-07-01", 0.02),
                (2, 2, "2026-07-01", 1000.0),
                (2, 3, "2026-07-02", 0.02),
                (2, 4, "2026-07-02", 0.02),
            ]
        )
        result = amount_temporal_support(tx)
        self.assertEqual(result[2]["temporal_matched_kzt"], 0.03)
        self.assertEqual(result[2]["temporal_matched_out_share"], 0.75)
    ...
    def test_seed_completeness_caps_flow_confidence_without_changing_priority(self):
        ...
        nodes.loc[nodes.gid.eq(BASE + 12), "depth"] = 1
        ...
        self.assertGreater(before.role_score, 0.65)
        self.assertEqual(after.role_score, 0.65)
        self.assertEqual(after.role_score_cap, 0.65)
...
class BetweennessTests(unittest.TestCase):
    ...
    def test_each_size_limit_switches_to_reproducible_sampling(self):
        ...
        for limit in ["exact_max_nodes", "exact_max_edges"]:
            config = replace(AnalysisConfig(), **{limit: 100})
            with self.subTest(limit=limit):
                actual, mode, sources = structural_betweenness(graph, config)
                self.assertEqual(mode, "sampled")
                self.assertEqual(sources, 256)
                self.assertEqual(
                    actual, nx.betweenness_centrality(graph, k=256, weight=None, seed=42)
                )
                ...
                self.assertEqual(actual, structural_betweenness(reversed_insertion, config)[0])
...
class AtomicOutputTests(unittest.TestCase):
    def setUp(self):
        ...
        self.graph = build_graph(self.e, self.n)
        self.save()
    ...
    def test_validation_and_json_failures_preserve_previous_generation(self):
        for target in ["hackalem.storage.validate_bundle", "starter.starter.write_graph_json"]:
            with (
                self.subTest(target=target),
                patch(target, side_effect=ValueError("injected failure")),
            ):
    ...
    def test_switch_failure_preserves_previous_generation(self):
        with patch("hackalem.output_store.os.replace", side_effect=OSError("switch failed")):
    ...
    def test_success_switches_csv_and_json_together_and_retains_old_snapshot(self):
        ...
        self.assertEqual(
            self.original, {p.name: p.read_bytes() for p in self.previous_version.iterdir()}
        )
    ...
    def test_validator_pins_a_generation_while_publisher_switches(self):
        ...
        def read_and_switch(handle, *args, **kwargs):
            nonlocal switched
            if not switched:
                switched = True
                self.save()
        ...
        with patch("hackalem.storage.pd.read_csv", read_and_switch):
            from hackalem.storage import validate_bundle

            roles, _, top, graph, manifest = validate_bundle(self.out, self.n, self.e)
        self.assertEqual(set(roles.evidence), {"in_deg=1; fixture only"})
        self.assertEqual(set(top.why), {"in_deg=1; fixture only"})
        self.assertEqual({row["evidence"] for row in graph["nodes"]}, set(roles.evidence))
    ...
    def test_unsupported_legacy_exchange_leaves_real_directory_untouched(self):
        ...
        self.out = legacy
        with patch(
            "hackalem.output_store.exchange_paths", side_effect=OSError("unsupported filesystem")
        ):
    ...
    def test_failed_first_generation_does_not_publish_partial_files(self):
        ...
        with patch("starter.starter.write_graph_json", side_effect=OSError("JSON write failed")):
